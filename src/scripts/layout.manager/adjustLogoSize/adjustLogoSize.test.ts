...
describe('adjustLogoSize', () => {
    ...
    beforeEach(() => {
        ...
        dm.setProperty(dm.logo, 'offsetHeight', 50)
    ...
    test('adjusts logo size for desktop view', () => {
        ...
        adjustLogoSize(dm)
        expect(dm.logo.classList.contains('logoImg-desktop')).toBe(true)
        expect(dm.logo.style.height).toBe('40px')
        expect(dm.logo.style.maxWidth).toBe('none')
    })
    test('does not add desktop class for mobile view', () => {
        dm.setProperty(dm.window, 'innerWidth', 500)
        adjustLogoSize(dm)
        expect(dm.logo.classList.contains('logoImg-desktop')).toBe(false)
    })
