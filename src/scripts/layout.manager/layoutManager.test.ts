...
describe('handleSearchInput', () => {
  ...
  beforeEach(() => {
    ...
    dm.searchInputs = dm.dom.window.document.querySelectorAll(
      'input[type="search"]'
    );
  });
  ...
  test('does nothing if searchInputs is empty', () => {
    dm.searchInputs =
      dm.dom.window.document.querySelectorAll('input[type="text"]'); // No search inputs
    handleSearchInput(dm);
...
describe('adjustDimensions', () => {
  test.each([
    ...
    (
      ...
      const [height, width, overflowY] = adjustDimensions(
        scrollHeight,
        isDesktopView,
        availableHeight,
        availableWidth
...
describe('setBreadcrumbTransparency', () => {
  ...
  test('removes transparent class when breadcrumbs are not empty', () => {
    ...
    setBreadcrumbTransparency(dm);
    expect(dm.breadcrumbContainer.classList.contains('transparent')).toBe(
      false
    );
  });
});
...
describe('whenNavEmpty', () => {
  ...
  test('handles desktop view when nav is empty', () => {
    Object.defineProperty(dm.window, 'innerWidth', {
      value: 800,
      writable: true,
    });
    whenNavEmpty(dm);
...
describe('initLayout', () => {
  ...
  beforeEach(() => {
    ...
    dm.dom.window.document.body.appendChild(dm.navbar);
    dm.dom.window.document.body.appendChild(dm.mainWrapper);
    Object.defineProperty(dm.window, 'addEventListener', {
      writable: true,
      value: vi.fn(),
    });
  });
  test('adjusts layout when logo is not loaded', () => {
    Object.defineProperty(dm.logoImg, 'complete', {
      value: false,
      writable: true,
    });
