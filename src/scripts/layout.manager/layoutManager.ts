...
import { isDesktopView, navempty, floadTitleWhenBreadcrumbsAllow, calculateAvailableDimensions } from "./mode/modeFactory"
...
export function adjustContentHeight(dm: IDataManager): void {
    ...
    const [height, width, overflowY, overflowX] = adjustDimensions(scrollHeight, isDesktopView_, availableHeight, availableWidth)
...
export function adjustDimensions(scrollHeight: number, isDesktopView_: boolean, availableHeight: number, availableWidth: number): [string, string, string, string] {
    const height = scrollHeight < availableHeight ? 'fit-content' : `${availableHeight}px`
...
export function resizeObserver(element: HTMLElement, callback: () => void): void {
