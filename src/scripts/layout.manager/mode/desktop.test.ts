...
class DesktopTest extends TestModes {
  ...
  testCalculateAvailableDimensions(dm: MockDataManager) {
    ...
    const { availableHeight, availableWidth } =
      this.testable.calculateAvailableDimensions(dm);
...
suite('Parameterized Tests', () => {
  ...
  const tests = [
    {
      name: 'testFloadTitleWhenBreadcrumbsAllow',
      method: (tc: DesktopTest, data: MockDataManager) =>
        tc.testFloadTitleWhenBreadcrumbsAllow(data),
    },
    {
      name: 'testCalculateAvailableDimensions',
      method: (tc: DesktopTest, data: MockDataManager) =>
        tc.testCalculateAvailableDimensions(data),
    },
    {
      name: 'handleView',
      method: (tc: DesktopTest, data: MockDataManager) =>
        tc.testHandleView(data),
    },
  ];
